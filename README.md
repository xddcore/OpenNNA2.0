 ...
 * @LastEditTime: 2022-08-27 20:05:23
...
|步骤|操作|软件|硬件平台|
|:----:|:----:|:----:|:----:|
|1️⃣|收集数据集，预处理数据集|DataAssitant(xddcore)|PC|
|2️⃣|目标检测数据集手动标注|Vott(Microsoft)|PC|
|3️⃣|训练神经网络(自动训练脚本，或将提供云端自动训练服务)|Tensorflow|PC(GPU)|
|4️⃣|Tensorflow Model 转 C Model File(若需量化，也在本步骤进行)|OpenNNA_Model_Converter.py|PC|
|5️⃣|部署到硬件平台运行|OpenNNA框架源码+C Model File|任意具有C编译器支持的硬件平台|
|6️⃣|根据不同的硬件平台，配置OpenNNA启用算子硬件加速，以获得更高的性能|||
|7️⃣|利用神经网络做一些具体应用(目标分类，目标检测，语音关键词识别等)|||
