 ...
 * @LastEditTime: 2022-08-15 22:25:10
...
3. 对于fpga:多加速器(NNA)调度，负载均衡｜目前计划支持的平台有正点院子ZYNQ-7020;PYNQ-Z2;Xlinx ZCU102
4. 提供算子开发框架，可自己开发自己的算子。为了独立性，一个算子一个.c文件
5. 提供demo: 1.手写数字 2.KWS(关键词识别) 3.yolo v1目标检测
6. 内存排列模式:CHW和HWC（经过试验，得益于优秀的流水线机制，在cortex m核上顺序读取内存空间和以一定规律间隔读取内存空间效率几乎相同)
7. 堆内存管理粒度:1.网络(四舍五入等于静态管理)2.网络层(算子)动态管理
8. 除了避免野指针等引发段错误，其他地方一律不设错误检测，按照demo的方式来调用/直接使用工具将H5/tflite模型文件转为C Model，理论上不会出错。
9. 提供两种网络释放方式:1.仅释放特征图堆内存(在这种释放方式下，重新调用Init函数可以恢复神神经网络计算)。2.释放整个网络的所有有关数据(例如网络结构，特征图堆内存等|网络权重由于是编译器分配的地址，无法通过OpenNNA来free。不过如果想要把这段内存用起来，可以直接对这些变量取址，然后用指针直接往里面去写入其他东西就行)(在这种释放方式下，网络所有信息将会永久从堆内存中被移除。恢复网络计算只能重启)
10. 提供不同硬件平台的算子硬件加速方案，如ARM的CMSIS-DSP/SIMD，FPGA的神经网络加速器等
