...
/* 静态特征图堆内存的分配策略(DYNAMIC_FMAP_HEAP=0):在OpenNNA_Init函数中统计最大的特征图输入堆内存和最大的特征图输出堆内存
 ...
 */
/* 动态堆内存的分配策略(DYNAMIC_FMAP_HEAP=1):在OpenNNA_Init函数中为第一层的输入特征图和输出特征图分配堆内存(为什么需要先分配第一层的堆内存，因为懒得重新写一套free Network逻辑了，
