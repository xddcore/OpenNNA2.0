    ...
    for (int i = 0; i < filters; ++i) {//filters must = Output_Fmap_Channel
        for (int j = 0; j < Output_Fmap_Row; ++j) {
            for (int k = 0; k < Output_Fmap_Col; ++k) {
                ...
                else{//kernel_channel!=3
                    for (int l = 0; l < kernel_channel; l=l+4) {
                        for (int m = 0; m < kernel_row; ++m) {
                            for (int n = 0; n < kernel_col; ++n) {
                                //输出特征图=输入特征图*卷积核权重
#if(HARDWARE_ACCELERATION==0)//不使用硬件加速,纯c推理(Float32)
...
#elif(HARDWARE_ACCELERATION==1)//不使用硬件加速,纯c推理(Int8)
                                Accumulator_INT32 += (((short int)((Fmap_t *)Layers->Input_Feature_Map)[(n+k*strides_col)+(m+j*strides_row)*Input_Fmap_Col+l*Input_Fmap_Col*Input_Fmap_Row])-Zi) * ((short int)((Weights_t *)Layers->Weights)[n+m*kernel_col+l*kernel_col*kernel_row+i*kernel_col*kernel_row*kernel_channel]-Zw);
                                Accumulator_INT32 += (((short int)((Fmap_t *)Layers->Input_Feature_Map)[(n+k*strides_col)+(m+j*strides_row)*Input_Fmap_Col+(l+1)*Input_Fmap_Col*Input_Fmap_Row])-Zi) * ((short int)((Weights_t *)Layers->Weights)[n+m*kernel_col+(l+1)*kernel_col*kernel_row+i*kernel_col*kernel_row*kernel_channel]-Zw);
                                Accumulator_INT32 += (((short int)((Fmap_t *)Layers->Input_Feature_Map)[(n+k*strides_col)+(m+j*strides_row)*Input_Fmap_Col+(l+2)*Input_Fmap_Col*Input_Fmap_Row])-Zi) * ((short int)((Weights_t *)Layers->Weights)[n+m*kernel_col+(l+2)*kernel_col*kernel_row+i*kernel_col*kernel_row*kernel_channel]-Zw);
                                Accumulator_INT32 += (((short int)((Fmap_t *)Layers->Input_Feature_Map)[(n+k*strides_col)+(m+j*strides_row)*Input_Fmap_Col+(l+3)*Input_Fmap_Col*Input_Fmap_Row])-Zi) * ((short int)((Weights_t *)Layers->Weights)[n+m*kernel_col+(l+3)*kernel_col*kernel_row+i*kernel_col*kernel_row*kernel_channel]-Zw);
#elif(HARDWARE_ACCELERATION==2)
                                //*
                            arm_mult_f32(&((Fmap_t *)Layers->Input_Feature_Map)[(n+k*strides_col)+(m+j*strides_row)*Input_Fmap_Col+l*Input_Fmap_Col*Input_Fmap_Row], \
                            &((Weights_t *)Layers->Weights)[n+m*kernel_col+l*kernel_col*kernel_row+i*kernel_col*kernel_row*kernel_channel], \
